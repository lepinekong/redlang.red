...
context [
		...
		switch type?/word value [
			file! [
				...
				all [
					...
					any [dir? value not rule parse value rule]
				...
				all [
					...
					if contents: attempt [read directory/:value][		; and it is not a fake directory
						...
						make-dir-tree/pref/dir/only-dirs/expand/dep/red/lin/filter contents copy changeprefix directory/:value dirs levels depth + 1 frm lins rule
					]
				]
			]
			block! [
				...
				foreach item value [
					...
					make-dir-tree/pref/chpref/dir/only-dirs/expand/dep/red/lin/filter item copy addprefix copy changeprefix directory dirs levels depth frm lins rule
				]
			]
		]
	]
	set 'dir-tree func [
		...
		if filter [append refs/2 'filter append/only refs rule]
		do reduce refs
		out
	]
]
